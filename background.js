...
async function takeSnapshot() {
  ...
  log("storage", "Snapshot saved to disk.");
}
...
async function restoreSnapshot() {
  ...
  if (!isLocked || Object.keys(currentSnapshot).length === 0) return;
  ...
  for (const rootId of Object.keys(currentSnapshot)) {
    try {
      const current = await browser.bookmarks.getChildren(rootId);
      await diffAndFix(rootId, current, currentSnapshot[rootId]);
    } catch (e) {
...
browser.runtime.onStartup.addListener(restoreSnapshot);
browser.runtime.onInstalled.addListener(restoreSnapshot);
...
(async () => {
  ...
  log("init", "Bookmark Guard Active and Persistent.");
})();
